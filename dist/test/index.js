...
const fixture = '🦄';
test.beforeEach(t => {
    t.context.config = new source_1.default({ cwd: tempy.directory() });
    t.context.configWithoutDotNotation = new source_1.default({ cwd: tempy.directory(), accessPropertiesByDotNotation: false });
});
test('.get()', t => {
...
test('.get() - `defaults` option', t => {
    const store = new source_1.default({
        cwd: tempy.directory(),
        defaults: {
            foo: 42,
            nested: {
...
test('with `suffix` option set to empty string', t => {
    ...
    const config = new source_1.default({ projectSuffix, projectName });
...
test('`projectName` option', t => {
    ...
    const config = new source_1.default({ projectName });
    t.is(config.get('foo'), undefined);
    config.set('foo', fixture);
    t.is(config.get('foo'), fixture);
...
test('ensure `.store` is always an object', t => {
    ...
    const config = new source_1.default({ cwd });
...
test('instance is iterable', t => {
    ...
    t.deepEqual([...t.context.config], [['foo', fixture], ['bar', fixture]]);
});
...
test('`cwd` option overrides `projectName` option', t => {
    const cwd = tempy.directory();
    t.notThrows(() => {
        const config = new source_1.default({ cwd, projectName: '' });
...
test('safely handle missing package.json', t => {
    ...
    let config;
    t.notThrows(() => {
        config = new source_1.default({ projectName: 'conf-fixture-project-name' });
        del.sync(config.path, { force: true });
    });
...
test('handle `cwd` being set and `projectName` not being set', t => {
    ...
    let config;
    t.notThrows(() => {
        config = new source_1.default({ cwd: 'conf-fixture-cwd' });
...
test('fallback to cwd if `module.filename` is `null`', t => {
    ...
    clearModule('.');
    t.notThrows(() => {
        const config = new source_1.default({ cwd: 'conf-fixture-fallback-module-filename-null' });
        del.sync(path.dirname(config.path));
...
test('encryption - upgrade', t => {
    const cwd = tempy.directory();
    const before = new source_1.default({ cwd });
    before.set('foo', fixture);
    t.is(before.get('foo'), fixture);
    const after = new source_1.default({ cwd, encryptionKey: 'abc123' });
    t.is(after.get('foo'), fixture);
});
test('encryption - corrupt file', t => {
    const cwd = tempy.directory();
    const before = new source_1.default({
        cwd,
        encryptionKey: 'abc123',
    ...
    const after = new source_1.default({
        cwd,
        encryptionKey: 'abc123',
        clearInvalidConfig: true
    });
...
test('doesn\'t write to disk upon instanciation if and only if the store didn\'t change', t => {
    ...
    t.is(exists, false);
    const conf = new source_1.default({
        cwd: tempy.directory(),
        defaults: {
            foo: 'bar'
        }
...
test('`clearInvalidConfig` option - invalid data', t => {
    const config = new source_1.default({ cwd: tempy.directory(), clearInvalidConfig: false });
...
test('schema - should be an object', t => {
    const schema = 'object';
    t.throws(() => {
        new source_1.default({ cwd: tempy.directory(), schema });
...
test('schema - valid set', t => {
    ...
    const config = new source_1.default({ cwd: tempy.directory(), schema });
    t.notThrows(() => {
...
test('schema - invalid write to config file', t => {
    ...
    const cwd = tempy.directory();
    const config = new source_1.default({ cwd, schema });
...
test('`watch` option watches for config file changes by another process', async (t) => {
    const cwd = tempy.directory();
    const conf1 = new source_1.default({ cwd, watch: true });
    const conf2 = new source_1.default({ cwd });
    conf1.set('foo', '👾');
    t.plan(4);
    const checkFoo = (newValue, oldValue) => {
        t.is(newValue, '🐴');
...
test('`watch` option watches for config file changes by file write', async (t) => {
    const cwd = tempy.directory();
    const conf = new source_1.default({ cwd, watch: true });
    conf.set('foo', '🐴');
    t.plan(2);
    const checkFoo = (newValue, oldValue) => {
        t.is(newValue, '🦄');
...
test('migrations - should save the project version as the initial migrated version', t => {
    const cwd = tempy.directory();
    const conf = new source_1.default({ cwd, projectVersion: '0.0.2', migrations: {} });
...
test('migrations - should save the project version when a migration occurs', t => {
    ...
    const conf = new source_1.default({ cwd, projectVersion: '0.0.2', migrations });
    t.is(conf.get('__internal__.migrations.version'), '0.0.2');
    const conf2 = new source_1.default({ cwd, projectVersion: '0.0.4', migrations });
...
test('migrations - should NOT run the migration when the version doesn\'t change', t => {
    ...
    const conf2 = new source_1.default({ cwd, projectVersion: '0.0.2', migrations });
...
test('migrations - should run the migration when the version changes', t => {
    ...
    t.is(conf.get('__internal__.migrations.version'), '0.0.2');
    t.false(conf.has('foo'));
    const conf2 = new source_1.default({ cwd, projectVersion: '1.1.0', migrations });
...
test('migrations - should run the migration when the version uses semver comparisons', t => {
    ...
    const conf = new source_1.default({ cwd, projectVersion: '1.0.2', migrations });
...
test('migrations - should run the migration when the version uses multiple semver comparisons', t => {
    ...
    t.is(conf.get('__internal__.migrations.version'), '1.0.2');
    t.is(conf.get('foo'), 'cool stuff');
    const conf2 = new source_1.default({ cwd, projectVersion: '2.0.1', migrations });
...
test('migrations - should run all valid migrations when the version uses multiple semver comparisons', t => {
    ...
    const conf = new source_1.default({ cwd, projectVersion: '2.4.0', migrations });
...
test('migrations - should NOT throw an error when project version is unspecified but there are no migrations', t => {
    const cwd = tempy.directory();
    t.notThrows(() => {
        const conf = new source_1.default({ cwd });
        conf.clear();
    });
});
...
test('migrations error handling - should rollback changes if a migration failed', t => {
    ...
    let conf = new source_1.default({ cwd, projectVersion: '1.0.0', migrations: passingMigrations });
    t.throws(() => {
        conf = new source_1.default({ cwd, projectVersion: '1.0.2', migrations: failingMigrations });
...
test('__internal__ keys - should not be accessible by the user even without dot notation', t => {
    const cwd = tempy.directory();
    const conf = new source_1.default({ cwd, accessPropertiesByDotNotation: false });
    t.throws(() => {
        conf.set({
            __internal__: {
                'you-shall': 'not-pass'
